...
namespace zks
{
    ...
    public:
        ...
        enum Column
        {
            DATETIME, EPOCHTIME, THREAD, FILE, FUNC, LINE, GROUP, LEVEL, MESSAGE, csize = 9
        };
        ...
        struct Config
        {
            ...
            struct Buff
            {
                ...
                size_t msg_max_size;
                Buff() :
                        enable(true), size(1000000), enable_line_buff(true), line_buff_size(1512), enable_msg_buff(true), msg_max_size(1000)
                {
                }
                ~Buff()
                {
            ...
            struct Output
            {
                ...
                Output() :
                        file_name("log.csv"), backup_period(0), backup_folder(".")
                {
                }
                ~Output()
                {
        ...
        public:
            ...
            Output output;
            Config() :
                    stream_type(2), thread_safe(1), verbosity(5), format(), buff(), output()
            {
            }
            ~Config()
            {
    ...
    inline simlog& endl(simlog& logger)
    {
        logger << "\n";
        return logger.flush();
    }
