...
namespace zks {
	...
	int simconf::_replace_var_(u8string* line, std::ostream* iop) {
		...
		for (size_t head = line->find("$("); head != u8string::npos; head = line->find("$(")) {
			...
			if (option(_sec_name_, from.substr(2, from.size() - 3), &to) < 0 
				&& option("global", from.substr(2, from.size() - 3), &to) < 0
			) {
				return -3;
			}
			if (iop) {
			...
			if (line) line->replace_all(from, to);
		}
		...
		for (std::wstring buff; getline(ifs, buff); ++ln) {
			...
			if (ret > 0) {
				if (_sec_map_.find(_sec_name_) != _sec_map_.end() && _sec_name_	!= "global") {
					if (iop) {
						(*iop) << _fname_ << "(" << ln << "): section ["
							<< _sec_name_ << "] re-define." << std::endl;
					}
					return -5;
				}
				if (iop) {
			...
			if (str_vec.size() != 2) {
				std::cerr << _fname_ << "(" << ln << "): option conf is not seperated by `" << _assign_
					<< "`." << std::endl;
				return -6;
			}
			else {
