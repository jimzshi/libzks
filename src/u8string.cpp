...
namespace unicode {
		...
		for (int lead_n = 3; lead_n < 8; ++lead_n) {
			if ((*s & lead_mask) == lead_control) {
				...
				if (len == 3) {
					...
					if (invalid_codepoint(wc)) break; //valid utf8 code but invalid unicode cp. code point in this area are reserved for utf16.
				}
				ret = true;
				break;
			}
	...
	u8string u8string::trim_spaces(std::locale const& loc) const {
		...
		size_type p{ 0 };
		while (p < ret.size() && std::isspace(ret[p], loc)) ++p;
		ret.erase(0, p);
